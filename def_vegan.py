def get_similar_vegan(foods):
    import pandas as pd
    import numpy as np
    from sklearn.metrics.pairwise import cosine_similarity

    vegan = pd.read_csv('vegan_similar.csv', index_col=0)

    if len(vegan[vegan['ingredients_new'].str.contains(foods.lower(),na=False)]) != 0:  # Eğer ingredientsta yoksa yemek ismine bak
        filtered_df = vegan[vegan['ingredients_new'].str.contains(foods.lower(), na=False)]
    else:
        filtered_df = vegan[vegan['food_name'].str.lower().str.contains(foods.lower(), na=False)]
    ...
    try:
        ...
        for j in ranked_foods:
            for i in vegan["food_name"]:
                if j == i:
                    df_output = pd.concat(
                        [df_output, vegan[vegan["food_name"] == i].loc[:, ['food_name', 'ingredients', 'recipe',
                                                                           'total_time_new', 'calories', 'protein_gr',
                                                                           'carbohydrates_gr', 'fat_gr',
                                                                           'cholesterol_mg',
                                                                           'sodium_mg']]])
