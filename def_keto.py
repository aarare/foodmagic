def get_similar_keto(foods):
    import pandas as pd
    import numpy as np
    from sklearn.metrics.pairwise import cosine_similarity

    keto = pd.read_csv('keto_similar.csv', index_col=0)

    if len(keto[keto['ingredients_new'].str.contains(foods.lower())]) != 0:  # Eğer ingredientsta yoksa yemek ismine bak
        filtered_df = keto[keto['ingredients_new'].str.contains(foods.lower())]
    else:
        filtered_df = keto[keto['food_name'].str.lower().str.contains(foods.lower())]
    ...
    try:
        ...
        for j in ranked_foods:
            for i in keto["food_name"]:
                if j == i:
                    df_output = pd.concat(
                        [df_output, keto[keto["food_name"] == i].loc[:, ['food_name', 'ingredients', 'recipe',
